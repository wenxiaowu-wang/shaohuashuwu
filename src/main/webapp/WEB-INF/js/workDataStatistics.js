let withdrawGoldCoinInterface_vm = new Vue({
    ...
    data:{
        ...
        tableData_display:[{
            withdraw_id: '斗破苍穹后传之大主宰',
            third_party_number: '第五百二十一章：陨落的天才',
            third_party_name: '用户名不宜太长',
    ...
    methods:{
        ...
        determineAmount(){
            ...
            }else if (this.select_value > this.goldCoin_able_withdraw){
                this.$message({
                    type: 'error',
        ...
        commitWithdrawForm(){
            ...
            if (this.withdrawForm.third_party_name === "请选择第三方平台" || this.third_party_number === ""){
                this.$message({
                    type:'error',
                    message:'请输入对应第三方账号，平台将自动打入金额到该账号。'
                });
            }else{
                //提交表单
                axios.post("/shaohuashuwu_war_exploded/transactionInfoController/withdrawGoldCoin/" +
                    this.user_id+"/"+this.select_value/10+"/"+this.withdrawForm.third_party_name+"/"+
    ...
    mounted(){
        ...
        axios.get("/shaohuashuwu_war_exploded/userSession/getUser").then(resp =>{
            ...
            //根据用户的ID获取该作者的作品数据
            axios.get("/shaohuashuwu_war_exploded/transactionSession/getTransactionGoldCoin").then(response =>{
