...
new Vue({
    ...
    methods:{
        ...
        submitForm(formName) {
            var _this= this;
            axios.post('http://localhost:8080/account/addworkInfo',this.worksInfo)
                .then(function (response) {
        ...
        isworkname(){
            ...
            if(!_this.worksInfo.work_name){
                ...
                _this.work_Tips.work_name_tip="1-15个汉字";
                document.getElementById("work_name_tipid").style.color = "red";
            }
            else{
                console.log(this.worksInfo.work_name)

                axios.post('http://localhost:8080/account/isworkname',_this.worksInfo)
                    .then(function (response){
                        console.log(response.data);
        ...
        isworkintroduct(){
            var _this=this;
            var isTel =/^[\u4e00-\u9fa5a-zA-Z\d,\.，。<>《》`~!@#$%^&*()_\-+=<>?:"{}|,.\/;'\\[\]·~！@#￥%……&*（）——\-+={}|《》？：“”【】、；‘'，。、]{20,500}$/;
        ...
        isworkotherwork(){
            ...
            if(!isTel.test(_this.worksInfo.work_other_word)){
                ...
                _this.work_Tips.work_otherword_tip="0~32个字，给读者展示你的版权页";
