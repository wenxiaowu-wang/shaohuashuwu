let messageCenterInterface_vm = new Vue({
    ...
    methods:{
        ...
        readOneMessage(message,index){
            //将一条对应消息置为已读
            axios.post("/shaohuashuwu_war_exploded/noticeStateInfoController/updateOneReadState/" +
        ...
        readAllMessage(type){
            //将对应类型通知消息置为已读
            axios.post("/shaohuashuwu_war_exploded/noticeStateInfoController/updateAllReadStateOfNoticeType/" +
                type).then(response =>{
                if(response.data){
                    this.$message({
                        type: 'success',
        ...
        deleteOneMessage(message,index){
          ...
          //删除一条对应消息ID（notice_id）的信息
            this.$confirm('此操作将删除该消息, 是否继续?', '提示', {
                ...
                //删除该条对应消息
                axios.post("/shaohuashuwu_war_exploded/noticeStateInfoController/updateOneDeleteState/" +
    ...
    mounted(){
        ...
        //获取所有该用户的通知消息
        axios.get("/shaohuashuwu_war_exploded/noticeInfoController/getAllNoticeInfo").then(resp =>{
