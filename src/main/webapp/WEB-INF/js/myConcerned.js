let attentionInterface_vm = new Vue({
    ...
    methods:{
        ...
        sendLetter(){
            ...
            this.$confirm('是否发送私信?', '提示', {
                ...
                //发送信件
                axios.post("/shaohuashuwu/noticeInfoController/addOnePrivateLetter/" +
        ...
        cancelAttention(id,name,index){
          ...
          //   let this_ = this;
            this.$confirm('此操作将取消关注[ '+name+' ], 是否继续?', '提示', {
                ...
                // let this__ = this;
                //删除关注信息
                axios.post("/shaohuashuwu/attentionInfoController/deleteAttentionInfo/" +
                    id).then(response =>{
                    this.$message({
                        type: 'success',
    ...
    mounted(){
        ...
        axios.get("/shaohuashuwu/attentionInfoController/" +
            ...
            this.displayDivsSetting();//判断是否显示数据为空提示
        }).catch(error =>{
            this.displayDivsSetting();//判断是否显示数据为空提示
