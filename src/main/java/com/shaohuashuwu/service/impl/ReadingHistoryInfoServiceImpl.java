...
public class ReadingHistoryInfoServiceImpl implements ReadingHistoryInfoService {
    ...
    ReadingHistoryInfo readingHistoryInfo;
    ...
    public Boolean addReadingHistoryInfo(int user_id, int work_id, Timestamp reading_time) {
        ...
        if (exist == 0) {
            int before = readingHistoryInfoDao.selectReadingHistoryInfoAllCountByUserId(user_id);
            System.out.println("点击开始阅读之前总数：" + before);

            readingHistoryInfoDao.insertReadingHistoryInfoByUserId(readingHistoryInfo);

            int after = readingHistoryInfoDao.selectReadingHistoryInfoAllCountByUserId(user_id);
            System.out.println("点击开始阅读之后总数：" + after);
            if (after > before) {
                inserResulte = true;
            }
        } else {
