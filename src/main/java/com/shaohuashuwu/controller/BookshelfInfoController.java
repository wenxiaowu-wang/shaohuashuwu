...
public class BookshelfInfoController {
    ...
    //移出书架
    @RequestMapping("/deleteBookshelfWorkByWorkId/{work_id}")
    @ResponseBody
    public Boolean deleteBookshelfWorkByWorkId(@PathVariable(value = "work_id") int work_id) {
        boolean theResult = false;
        if (bookshelfInfoService.deleteBookshelfWorkByWorkId(work_id)) {
            theResult = true;
        }
        return theResult;
    }
